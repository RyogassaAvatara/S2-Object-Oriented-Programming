...
public class NBAManager implements LeagueManager {
    ...
    private void addTeam(){
        if(league.size() == numberOfTeams) {
            System.out.println("INVALID...Unable to Add more Teams to the league!");
            return;
        }
        ...
        String line = scanner.nextLine();
         for(BasketballTeam Team : league) {
             if(Team.getName().equals(line)){
                 league.remove(Team);
                 System.out.println(Team.getName()+" is removed from the League!");
                 return;
             }
         }
    ...
    // This will Display the table when user input it
